...
if __name__ == '__main__':
   ...
   # --- Part 1: Backtest and Tune Models on the 2024 Season ---
    ...
    # --- Part 2: Train Final Models with Best Parameters and Predict 2025 ---
    print("\n" + "="*50)
    print("PART 2: TRAINING FINAL MODELS & PREDICTING WEEK 1, 2025")
    print("="*50)
    ...
    # --- Train and Predict with Final Tuned Random Forest Model ---
    if best_rf_params:
        print("\n--- Training Final RandomForest Model with Tuned Hyperparameters ---")
        final_rf_model = RandomForestClassifier(random_state=42, class_weight='balanced').set_params(**best_rf_params)
        final_rf_model.fit(X_full, y_full)
        print("RandomForest Model Training Complete.")
        final_week_1_rf = predict_touchdown_scorers(feature_df, final_rf_model, features,
                                  year=2025, week=1, future_odds_df=future_odds_df)
        final_week_1_rf.to_csv('week_1_predictions_rf', index=False)

    # --- Train and Predict with Final Tuned LightGBM Model ---
    if best_lgbm_params:
        print("\n--- Training Final LightGBM Model with Tuned Hyperparameters ---")
        final_lgbm_model = lgb.LGBMClassifier(objective='binary', random_state=42, is_unbalance=True, verbosity=-1).set_params(**best_lgbm_params)
        final_lgbm_model.fit(X_full, y_full)
        print("LightGBM Model Training Complete.")
        final_week_1_lgbm = predict_touchdown_scorers(feature_df, final_lgbm_model, features,
                                  year=2025, week=1, future_odds_df=future_odds_df)
        final_week_1_lgbm.to_csv('week_1_predictions_lgbm', index=False)