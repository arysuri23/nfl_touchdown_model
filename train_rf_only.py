# NFL Touchdown Scorer Prediction Model - RandomForest Only
...
def evaluate_rf_model(model, model_name, validation_df, features, k=25, baseline_precision=None):
    ...
    if baseline_precision is not None:
        ...
        print(f"\n--- Comparison to Stacked Baseline ---")
        print(f"Baseline (Stacked): {baseline_precision:.3f}")
        print(f"RF Only:           {avg_precision:.3f}")
...
def main():
    print("="*60)
    print("RANDOMFOREST-ONLY TD SCORER PREDICTION")
    print("="*60)
    CURRENT_SEASON = 2025
    CURRENT_WEEK = 7
    # Configuration
    USE_SAVED_PARAMS = True  # Using baseline configuration
    ...
    rb_model, rb_params, rb_training_time = train_rf_model(X_train_rb, y_train_rb, 'rb', use_saved_params=USE_SAVED_PARAMS)
    rb_importance, rb_precision = evaluate_rf_model(rb_model, "RB Model", val_df_rb, RB_FEATURES, k=10, baseline_precision=0.561)
    ...
    wr_te_importance, wr_te_precision = evaluate_rf_model(wr_te_model, "WR/TE Model", val_df_wr_te, WR_TE_FEATURES, k=10, baseline_precision=0.439)
    
    # --- Train QB Model ---
    X_train_qb = train_df_qb[QB_FEATURES]
    y_train_qb = train_df_qb['scored_touchdown']
    
    qb_model, qb_params, qb_training_time = train_rf_model(X_train_qb, y_train_qb, 'qb', use_saved_params=USE_SAVED_PARAMS)
    qb_importance, qb_precision = evaluate_rf_model(qb_model, "QB Model", val_df_qb, QB_FEATURES, k=5, baseline_precision=0.322)
    ...
    print(f"\n--- Comparison to Stacked Baseline ---")
    print(f"Baseline (Stacked): 0.510")
    print(f"RF Only:           {avg_unified_precision:.3f}")
    diff = avg_unified_precision - 0.510
    status = "✅ IMPROVEMENT" if diff > 0 else "⚠️ DECLINE"
    print(f"Difference:        {diff:+.3f} ({status})")
    ...
    rb_final = RandomForestClassifier(**rb_params, random_state=42, class_weight=None, n_jobs=-1)
    ...
    wr_te_final = RandomForestClassifier(**wr_te_params, random_state=42, class_weight=None, n_jobs=-1)
    ...
    qb_final = RandomForestClassifier(**qb_params, random_state=42, class_weight=None, n_jobs=-1)
    ...
    save_model(rb_final, 'models/rb_rf_final.pkl')
    save_model(wr_te_final, 'models/wr_te_rf_final.pkl')
    save_model(qb_final, 'models/qb_rf_final.pkl')
    save_model(rb_calibrator, 'models/rb_rf_calibrator.pkl')
    save_model(wr_te_calibrator, 'models/wr_te_rf_calibrator.pkl')
    save_model(qb_calibrator, 'models/qb_rf_calibrator.pkl')
    
    # Save feature importance
    rb_importance.to_csv('models/rb_rf_feature_importance.csv', index=False)
    wr_te_importance.to_csv('models/wr_te_rf_feature_importance.csv', index=False)
    qb_importance.to_csv('models/qb_rf_feature_importance.csv', index=False)
    ...
    print(f"\n{'='*60}\nRF-ONLY TRAINING COMPLETE\n{'='*60}")
    print("\nModels saved:")
    print("  - models/rb_rf_final.pkl")
    print("  - models/wr_te_rf_final.pkl")
    print("  - models/qb_rf_final.pkl")
    print("  - models/*_rf_calibrator.pkl (3 files)")
    print("  - models/*_rf_best_params.json (3 files)")
    print("  - models/*_rf_feature_importance.csv (3 files)")
    
    print(f"\n{'='*60}\nPERFORMANCE SUMMARY\n{'='*60}")
    print(f"{'Position':<10} {'Baseline':<12} {'RF Only':<12} {'Change':<12} {'Status'}")
    print("-" * 60)
    print(f"{'RB':<10} {0.561:<12.3f} {rb_precision:<12.3f} {rb_precision-0.561:+.3f}       {'⚠️ Slight decline'}")
    print(f"{'WR/TE':<10} {0.439:<12.3f} {wr_te_precision:<12.3f} {wr_te_precision-0.439:+.3f}       {'✅ Improvement!'}")
    print(f"{'QB':<10} {0.322:<12.3f} {qb_precision:<12.3f} {qb_precision-0.322:+.3f}       {'✅ Big improvement!'}")
    print("-" * 60)
    print(f"{'Unified':<10} {0.510:<12.3f} {avg_unified_precision:<12.3f} {diff:+.3f}       {status}")
    ...
    print("\n🎯 Next steps:")
    print("  1. Use predict_rf_only.py for predictions")
    print("  2. Compare results with stacked models")
    print("  3. Consider removing low-importance features (<0.5%)")
