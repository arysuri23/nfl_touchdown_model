...
def predict_touchdown_scorers(feature_df, models, calibrators, year, week, future_odds_df, td_odds_df):
    """Predicts touchdown scorers using loaded LightGBM-only models."""
    ...
    # Use simple predict_proba (no stacking)
    pred_df_rb['predicted_touchdown_probability'] = models['rb'].predict_proba(pred_df_rb[RB_FEATURES])[:, 1]
    pred_df_wr_te['predicted_touchdown_probability'] = models['wr_te'].predict_proba(pred_df_wr_te[WR_TE_FEATURES])[:, 1]
    pred_df_qb['predicted_touchdown_probability'] = models['qb'].predict_proba(pred_df_qb[QB_FEATURES])[:, 1]
...
if __name__ == '__main__':
    """
    Main entry point for LightGBM-only predictions.
    """
    print("="*60)
    print("LIGHTGBM-ONLY TD PREDICTION")
    print("="*60)
    
    prediction_year = 2025
    prediction_week = 7  # Update this for each week
    
    # --- 1. Load LightGBM-Only Models ---
    print("\nLoading LightGBM-only model artifacts from local files...")
    models = {
        'rb': load_joblib_locally('models/rb_lgbm_final.pkl'),
        'wr_te': load_joblib_locally('models/wr_te_lgbm_final.pkl'),
        'qb': load_joblib_locally('models/qb_lgbm_final.pkl')
    }
    calibrators = {
        'rb': load_joblib_locally('models/rb_lgbm_calibrator.pkl'),
        'wr_te': load_joblib_locally('models/wr_te_lgbm_calibrator.pkl'),
        'qb': load_joblib_locally('models/qb_lgbm_calibrator.pkl')
    }
    ...
    # --- 3. Run Prediction ---
    print(f"\nGenerating predictions for {prediction_year}, Week {prediction_week}...")
    ...
    final_predictions_df.to_csv(f'data/predictions_week_{prediction_week}_lgbm.csv', index=False)
    
    print(f"\n{'='*60}")
    print(f"TOP 20 TD PREDICTIONS - WEEK {prediction_week}")
    print("="*60)
    ...
    print(f"\n✓ Predictions saved to data/predictions_week_{prediction_week}_lgbm.csv")
