# NFL Touchdown Scorer Prediction Model - XGBoost Only
# Simplified version using only XGBoost (no stacking)
...
# --- 3. XGBoost Training Function ---
def train_xgb_model(X_train, y_train, position_name, use_saved_params=False):
    """Trains a single XGBoost model with optional hyperparameter tuning."""
    
    print(f"\n{'='*60}\nTRAINING XGBOOST MODEL: {position_name}\n{'='*60}")
    
    start_time = time.time()
    param_file = f'models/{position_name}_xgb_best_params.json'
    ...
    if use_saved_params and os.path.exists(param_file):
        ...
        xgb = XGBClassifier(random_state=42, eval_metric='logloss', tree_method='hist', n_jobs=-1)
        
        # Use TimeSeriesSplit instead of standard K-Fold to respect temporal order
        tscv = TimeSeriesSplit(n_splits=5)
        
        random_search = RandomizedSearchCV(
            xgb, XGB_PARAM_DIST, n_iter=20, cv=tscv, scoring='average_precision',
    ...
    final_xgb = XGBClassifier(**best_params, random_state=42, eval_metric='logloss', tree_method='hist', n_jobs=-1)
    final_xgb.fit(X_train, y_train)
    
    training_time = time.time() - start_time
    print(f"✓ Training complete in {training_time:.1f} seconds ({training_time/60:.1f} minutes)")
    
    return final_xgb, best_params, training_time


# --- 4. Evaluation Function ---
def evaluate_xgb_model(model, model_name, validation_df, features, k=25, baseline_precision=None):
    """Evaluates a single XGBoost model."""
    ...
    if baseline_precision is not None:
        ...
        print(f"XGBoost:       {avg_precision:.3f}")
...
def main():
    print("="*60)
    print("XGBOOST-ONLY TD SCORER PREDICTION")
    print("="*60)
    CURRENT_SEASON = 2025
    CURRENT_WEEK = 7
    # Configuration
    ...
    rb_model, rb_params, rb_training_time = train_xgb_model(X_train_rb, y_train_rb, 'rb', use_saved_params=USE_SAVED_PARAMS)
    rb_importance, rb_precision = evaluate_xgb_model(rb_model, "RB Model", val_df_rb, RB_FEATURES, k=10, baseline_precision=0.539)
    
    # --- Train WR/TE Model ---
    X_train_wr_te = train_df_wr_te[WR_TE_FEATURES]
    y_train_wr_te = train_df_wr_te['scored_touchdown']
    
    wr_te_model, wr_te_params, wr_te_training_time = train_xgb_model(X_train_wr_te, y_train_wr_te, 'wr_te', use_saved_params=USE_SAVED_PARAMS)
    wr_te_importance, wr_te_precision = evaluate_xgb_model(wr_te_model, "WR/TE Model", val_df_wr_te, WR_TE_FEATURES, k=10, baseline_precision=0.411)
    
    # --- Train QB Model ---
    X_train_qb = train_df_qb[QB_FEATURES]
    y_train_qb = train_df_qb['scored_touchdown']
    
    qb_model, qb_params, qb_training_time = train_xgb_model(X_train_qb, y_train_qb, 'qb', use_saved_params=USE_SAVED_PARAMS)
    qb_importance, qb_precision = evaluate_xgb_model(qb_model, "QB Model", val_df_qb, QB_FEATURES, k=5, baseline_precision=0.356)
    ...
    print(f"RandomForest: 0.542")
    print(f"LightGBM:     0.559")
    print(f"XGBoost:      {avg_unified_precision:.3f}")
    diff_rf = avg_unified_precision - 0.542
    diff_lgbm = avg_unified_precision - 0.559
    print(f"Difference vs RF:   {diff_rf:+.3f}")
    print(f"Difference vs LGBM: {diff_lgbm:+.3f}")
    ...
    rb_final = XGBClassifier(**rb_params, random_state=42, eval_metric='logloss', tree_method='hist', n_jobs=-1)
    ...
    wr_te_final = XGBClassifier(**wr_te_params, random_state=42, eval_metric='logloss', tree_method='hist', n_jobs=-1)
    ...
    qb_final = XGBClassifier(**qb_params, random_state=42, eval_metric='logloss', tree_method='hist', n_jobs=-1)
    ...
    save_model(rb_final, 'models/rb_xgb_final.pkl')
    save_model(wr_te_final, 'models/wr_te_xgb_final.pkl')
    save_model(qb_final, 'models/qb_xgb_final.pkl')
    save_model(rb_calibrator, 'models/rb_xgb_calibrator.pkl')
    save_model(wr_te_calibrator, 'models/wr_te_xgb_calibrator.pkl')
    save_model(qb_calibrator, 'models/qb_xgb_calibrator.pkl')
    
    # Save feature importance
    rb_importance.to_csv('models/rb_xgb_feature_importance.csv', index=False)
    wr_te_importance.to_csv('models/wr_te_xgb_feature_importance.csv', index=False)
    qb_importance.to_csv('models/qb_xgb_feature_importance.csv', index=False)
    ...
    print(f"\n{'='*60}\nXGBOOST-ONLY TRAINING COMPLETE\n{'='*60}")
    print("\nModels saved:")
    print("  - models/rb_xgb_final.pkl")
    print("  - models/wr_te_xgb_final.pkl")
    print("  - models/qb_xgb_final.pkl")
    print("  - models/*_xgb_calibrator.pkl (3 files)")
    print("  - models/*_xgb_best_params.json (3 files)")
    print("  - models/*_xgb_feature_importance.csv (3 files)")
    
    print(f"\n{'='*60}\nPERFORMANCE SUMMARY\n{'='*60}")
    print(f"{'Position':<10} {'RF':<12} {'LGBM':<12} {'XGBoost':<12} {'Best'}")
    print("-" * 60)
    print(f"{'RB':<10} {0.539:<12.3f} {0.617:<12.3f} {rb_precision:<12.3f} {'LGBM' if 0.617 >= max(0.539, rb_precision) else ('XGB' if rb_precision > 0.539 else 'RF')}")
    print(f"{'WR/TE':<10} {0.411:<12.3f} {0.417:<12.3f} {wr_te_precision:<12.3f} {'LGBM' if 0.417 >= max(0.411, wr_te_precision) else ('XGB' if wr_te_precision > 0.411 else 'RF')}")
    print(f"{'QB':<10} {0.356:<12.3f} {0.344:<12.3f} {qb_precision:<12.3f} {'RF' if 0.356 >= max(0.344, qb_precision) else ('XGB' if qb_precision > 0.344 else 'LGBM')}")
    print("-" * 60)
    print(f"{'Unified':<10} {0.542:<12.3f} {0.559:<12.3f} {avg_unified_precision:<12.3f}")
    ...
    print("\n🎯 Next steps:")
    print("  1. Compare XGBoost vs LightGBM vs RandomForest")
    print("  2. Consider ensemble or hybrid approach")
    print("  3. Ship best model for Week 5 predictions")
