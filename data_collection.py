...
def get_nfl_data(years):
    ...
    df = df.filter(pl.col('position').is_in(['QB', 'RB', 'TE', 'WR']))
...
def get_redzone_data(pbp):
    ...
    team_rz_plays = (
        ...
        .agg([
            pl.col('rush_attempt').sum().alias('team_rz_rushes'),
    ...
    final_rz = final_rz.with_columns([
        pl.when(pl.col('team_rz_rushes') > 0)
          .then((pl.col('player_rz_rushes').fill_null(0) / pl.col('team_rz_rushes')))
          .otherwise(0.0)
          .alias('redzone_carry_share'),
    ...
    return final_rz.select([
        'player_id', 'season', 'week','redzone_carry_share', 'redzone_target_share'
    ]).to_pandas()
...
def get_goal_line_data(pbp):
    ...
    team_gl_plays = goal_line_df.group_by(['posteam', 'season', 'week']).agg([
        pl.col('rush_attempt').sum().alias('team_gl_rushes'),
    ...
    final_gl = final_gl.with_columns([
        pl.when(pl.col('team_gl_rushes') > 0)
          .then((pl.col('player_gl_rushes').fill_null(0) / pl.col('team_gl_rushes')))
          .otherwise(0.0)
          .alias('inside_5_carry_share'),
    ...
    return final_gl.select([
        'player_id', 'season', 'week', 'inside_5_carry_share', 'inside_5_target_share'
    ]).to_pandas()
...
def get_opponent_positional_data(pbp, rosters):
    ...
    pass_cols_rename = {col: f'passing_tds_allowed_to_{col}' 
                        ...
                        if col not in ['defteam', 'season', 'week']}
    pass_tds_allowed = pass_tds_allowed.rename(pass_cols_rename)
...
def get_team_play_data(years):
    ...
    team_data = team_data.select(['season', 'week', 'team', 'pass_rate', 'rush_rate'])
