...
def get_depth_chart_data(years):
    ...
    depth_df = depth_df[['player_id', 'season', 'week', 'depth_chart_rank']]
    # Deduplicate per player-week to avoid exploding rows during merges
    depth_df = depth_df.drop_duplicates(subset=['player_id', 'season', 'week'], keep='last')
    return depth_df
...
def get_snap_counts(years):
    ...
    # Ensure one row per name-week to prevent duplicate merges
    snap_df = snap_df.drop_duplicates(subset=['merge_name', 'season', 'week'], keep='last')
...
def get_all_historic_data(years, team_map):
    ...
    years_no_2025 = [y for y in years if y != 2025]
    nfl_df = get_nfl_data(years_no_2025)
    
    # Only load 2025 weekly data if 2025 is in requested years
    if 2025 in years:
    ...
    # Only load 2025 depth chart data if 2025 is in requested years
    if 2025 in years:
    ...
    nfl_df = pd.merge(nfl_df, snap_counts_df, on=['merge_name', 'season', 'week'], how='left')
    ...
    # Enforce uniqueness: one row per player-week to guarantee correct rolling behavior
    nfl_df = nfl_df.sort_values(by=['season', 'week', 'player_id'], ignore_index=True)
    nfl_df = nfl_df.drop_duplicates(subset=['player_id', 'season', 'week'], keep='last').reset_index(drop=True)
